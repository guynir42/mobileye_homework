...
def test_ingest_detections():
    ...
    with SmartSession() as session:
        ...
        assert (
            len(vehicle.detections) >= 7
        )  # there is no deduplication on detections, so past test may have added some

        for time, objs in zip(times, objects):
            det = session.scalars(
                sa.select(Detection).where(Detection.timestamp == time)
            ).all()
            for obj in objs:
                assert any(
                    [d.type == obj for d in det]
                )  # this also verifies we have enough detections
            assert all(
                [d.vehicle == vehicle for d in det]
            )  # make sure all detections are for the right vehicle
    ...
    with SmartSession() as session:
        for vid, time, status in zip(vids, times, statuses):
            ...
            vehicles = session.scalars(
                sa.select(Vehicle).where(Vehicle.id == vid)
            ).all()
            assert (
                len(vehicles) == 1
            )  # deduplication should make sure no vehicles are duplicated in the DB
            assert vehicles[0].id == vid
            assert (
                len(vehicles[0].reports) > 0
            )  # no deduplication on reports, so there could be more than one
            assert all(
                [rep.status == status for rep in vehicles[0].reports]
            )  # make sure all reports are as expected

            reports = session.scalars(
                sa.select(Report).where(Report.vehicle_id == vid)
            ).all()
            assert (
                len(reports) >= 1
            )  # no deduplication on reports, so there could be more than one
            ...
            assert all(
                [rep.status == status for rep in reports]
            )  # make sure all reports are as expected
...
def test_illegal_report_type():
    ...
    data = dict(
        vehicle_wrong_key=data["vehicle_status"]
    )  # copy of the data but with wrong top-level key
