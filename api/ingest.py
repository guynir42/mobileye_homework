...
def ingest(data, session=None):
    ...
    try:
        ...
        if 'object_detection_events' in data_dict.keys():
            ingest_detections(data_dict['object_detection_events'], status_report, session=session)
        ...
        with SmartSession(session) as session:
            for report in report_list:
                try:
                    ...
                    db_report = Report(vehicle=vehicle, status=report['status'], timestamp=report['timestamp'])
                ...
                try:
                    ...
                    for detection in event['detections']:
                        try:
                            ...
                            status_report['status'] = 'failure'
                            status_report['errors'].append(f'Could not save detection: {e}')
                            return
